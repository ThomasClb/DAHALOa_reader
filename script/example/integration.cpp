...
const AlgebraicMatrix<double> get_B() {
    DACE::AlgebraicMatrix<double> B(13, 12);
...
double normtmp(int const& N, vector<double> const& X) {
...
// Classic integrators
AlgebraicVector<DA> RK78(
    int const& N, vectorDA Y0,
    double const& X0, double const& X1,
    vectorDA(*f)(vectorDA const&, double const&)) {

    double ERREST;
    double H0 = MIN_STEP; double HS = 0.1; double H1 = 100.0;
    double BS = 20 * EPS;

    AlgebraicMatrix<DA> Z(N, 16);
    vectorDA Y1(N);
    ...
    while (X != X1) {
        ...
        if (abs(H) > abs(HH1)) { H = HH1; }
            ...
            H = HH0;
            std::cout << X << " --- WARNING, MINIMUM STEPSIZE REACHED IN RK" << std::endl;
        }
...
AlgebraicVector<double> RK78(
    int const& N, vectordb Y0,
    double const& X0, double const& X1,
    vectordb(*f)(vectordb const&, double const&)) {
